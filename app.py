...
def get_breeds_with_id(livestockId):
...
def delete_livestock_selection_item():
    data = request.get_json()
    filterId = data.get('itemToDelete')
...
def produce_get_user(user1):
    user_id = user1
    produce = db.session.query(Produce).filter(
        Produce.user_id == user_id).all()
    output = []
    for item_produce in produce:
        item_produce_data = {}
        x = item_produce.id
